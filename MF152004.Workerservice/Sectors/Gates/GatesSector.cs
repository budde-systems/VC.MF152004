...
namespace MF152004.Workerservice.Sectors.Gates
{
    public abstract class GatesSector : Sector
    {
        ...
        private bool ValidShippedStatus(int shipmentId, Toward toward)
        {
            ...
            {
                string msg = $"The package with ID {shipmentId} has the wrong status";
        ...
        public override void UnsubscripedPacket(object? sender, UnsubscribedPacketEventArgs unsubscribedPacket)
        {
            ...
            {
                int? shipmentId = _contextService.GetShipmentByPacketTracing(unsubscribedPacket.PacketTracing)?.Id;

                if (shipmentId != null)
                {
                    string? target = GetDestinationOfTrackedPacket(unsubscribedPacket.PacketTracing);
