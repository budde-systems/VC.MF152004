...
public class SectorServices
{
    ...
    public void RunService(List<Sector> sectors)
    {
        _sectors = sectors;
        SubscribeEvents();
        _logger.LogInformation("SectorServices has been started successfully");
    }
    ...
    private void SubscribeEvents()
    {
        if (_sectors != null && _sectors.Any())
        {
            ...
            _logger.LogError("No sectors could be subscribed in SectorServices");
    }
    ...
    private void RemoveGhostTrackedPackages(TrackedPacket tracking) //remove packages that have not been ejected
    {
        ...
        {
            _logger.LogWarning("The remove-ghost-tracked-packages could not be executed. The shipment ID is less than 1");
            return;
        }
        ...
        foreach (var sector in _sectors)
        {
            ...
            try
            {
                ...
                {
                    _logger.LogInformation("A tracked shipment which is located in sector {0} has been detected in sector {1}. This will be removed", tracking.SectorName, sector);
